...
public class PmAccountController extends Controller{
	
	private static final Logger logger = Logger.getLogger(PmAccountController.class);
	
	//配置路由
	public void index() {

		this.renderJsp("account/pmAccount/pmAccount.jsp");
		 
			
	}
	
	public void staffDataShow() throws SQLException{
		
		try {
			List<Record> accountList = Db.find("SELECT STAFF_ID,USERNAME AS ACCOUNT,STAFF_NAME,STAFF_TYPE,STATE,CREATE_DATE"
					+ " FROM OA_STAFF");
			renderJson(accountList);
		} catch (Exception e) {
